...
export function readGitHeadSha(): string | undefined {
  try {
...
export function resolveHtpBuildSha(
  env: BuildShaEnvironment = process.env,
  readGitSha: () => string | undefined = readGitHeadSha,
): string {
...
export function createHtpBuildVersion(
  ...
  const normalizedSha = normalizeFullGitSha(sha) ?? (sha === "unknown" ? "unknown" : "unknown");
  return {
    app: HTP_APP_NAME,
    sha: normalizedSha,
...
export function createHtpBuildVersionPlugin(
  ...
  const sha = resolveHtpBuildSha(options.env ?? process.env, options.readGitSha ?? readGitHeadSha);
