...
--
-- This candidate is NOT an active managed migration. It lives under
-- supabase/migration-candidates/ and is never picked up by the migration
-- runner. Promotion to supabase/migrations/ and live application are
-- separate, explicitly authorized phases.
--
-- Scope: narrow CREATE OR REPLACE of exactly one function —
--   public.claim_recruiter_checkout_intent(uuid,uuid,text)
--
-- Everything else from the production definition
-- (supabase/migrations/20260730060000_phase1r_d2_b1_recruiter_checkout_intents.sql)
-- is preserved verbatim: exact signature, exact RETURNS TABLE columns,
-- ownership check, suspension-check-before-readiness ordering, advisory
-- lock keyed by recruiter_id, fixed 300-second lease, full state machine,
-- generation matrix, customer/session handling, SECURITY DEFINER, and
-- SET search_path = public.
--
-- The ONLY behavioral change is that the obsolete approved-verification
-- gate is replaced with the canonical readiness helper
-- public.recruiter_profile_can_manage_opportunities(uuid), matching the
-- edge adapter and the rest of the recruiter surface.
--
-- No table creation, no DML/backfill, no policy, no trigger, no DROP, no
-- destructive action, no edits to historical migration files.
