-- CANDIDATE MIGRATION — Phase RB-2D.
-- Driver HaulTracker message → recruiter Telegram delivery.
--
-- Scope contract:
...
--
-- Deliberately NOT here: no read receipts back to the driver, no group
-- delivery, no media, no AI, no conversation session state, no second
-- conversation system, no webhook, no second poller, no credential.
...
CREATE TABLE IF NOT EXISTS public.telegram_conversation_message_deliveries (
  ...
  -- Durable at-most-once identity: one delivery per message + recipient.
...
-- A delivered bot message is uniquely addressable by (chat, message id), so an
-- inbound recruiter reply can never resolve to two threads.
...
DECLARE
  ...
  _body text;
BEGIN
  -- 2a. Materialise eligible work. Pure read of the CF-1 conversation system;
  -- no conversation row is written.
  --
  -- Eligibility, all required together:
  --   * thread is currently 'active' and has a canonical acceptance time;
  --   * the message sender is EXACTLY the thread's driver (no echo);
  --   * the message was created strictly AFTER acceptance (no backlog);
  --   * the recipient already received a SENT RB-2A alert for that thread;
  --   * the recipient is still authorized right now.
  FOR _eligible IN
  ...
  -- 2b. Claim pending work ONLY. RB-2A.1 ambiguity hardening: a row left
  -- 'claimed' may already have been delivered by Telegram, so it is never
  -- auto-reclaimed. Authorization and Telegram linkage are re-resolved HERE.
  FOR _row IN
  ...
  LOOP
    ...
    -- Canonical body, read at claim time. Never copied into this table.
...
-- Only a CONFIRMED Telegram success may reach 'sent'. The delivered message id
-- is persisted so the recruiter can reply to THAT message (see section 4).
...
-- A KNOWN send failure returns the row to 'pending' so the next cycle retries
-- it, and gives up after a bounded number of attempts. It never marks delivery.
...
-- ─────────────────────────────────────────────────────────────────────────────
--
-- IDENTICAL to RB-2C in every respect except thread resolution, which now
-- accepts EITHER a sent conversation alert OR a sent driver-message delivery,
-- in both cases addressed to the acting account in its own private chat. A
-- copied message id from another user or chat still resolves to nothing.
...
BEGIN
  ...
  ELSE
    ...
    ELSE
      ...
      -- Locator source 2 (RB-2D): a delivered driver message. Same actor, same
      -- private chat, confirmed sent. No other routing source exists.
