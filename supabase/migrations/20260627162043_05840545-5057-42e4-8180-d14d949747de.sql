...
CREATE TABLE public._phase3c_results (test text, expected text, actual text, pass boolean);
GRANT SELECT ON public._phase3c_results TO authenticated, anon, service_role;
...
BEGIN
  ...
  IF _opp IS NULL THEN SELECT id, recruiter_id INTO _opp, _rec FROM public.opportunities LIMIT 1; END IF;

  INSERT INTO auth.users (id,email,instance_id,aud,role) VALUES
    (_referrer,'p3c2-referrer@test.local','00000000-0000-0000-0000-000000000000','authenticated','authenticated'),
    (_ref2,'p3c2-ref2@test.local','00000000-0000-0000-0000-000000000000','authenticated','authenticated'),
    (_ap1,'p3c2-ap1@test.local','00000000-0000-0000-0000-000000000000','authenticated','authenticated'),
    (_ap2,'p3c2-ap2@test.local','00000000-0000-0000-0000-000000000000','authenticated','authenticated'),
    (_ap3,'p3c2-ap3@test.local','00000000-0000-0000-0000-000000000000','authenticated','authenticated'),
    (_ap4,'p3c2-ap4@test.local','00000000-0000-0000-0000-000000000000','authenticated','authenticated');
  INSERT INTO public.admin_users (user_id,email,role) VALUES
    (_referrer,'p3c2-referrer@test.local','admin'),
    (_ref2,'p3c2-ref2@test.local','admin') ON CONFLICT DO NOTHING;
  ...
  VALUES (_opp,_rec,_referrer,_ap1,'referral_sent') RETURNING id INTO _ref_linked;
  ...
  INSERT INTO public._phase3c_results VALUES ('A linked referral advances','application_started',_status,_status='application_started');
  SELECT count(*) INTO _events FROM public.referral_status_events WHERE referral_id=_ref_linked;
  INSERT INTO public._phase3c_results VALUES ('A events written >=2','>=2',_events::text,_events>=2);

  INSERT INTO public.driver_referrals(opportunity_id,recruiter_id,referring_driver_id,referred_driver_email,status)
  VALUES (_opp,_rec,_referrer,'p3c2-ap2@test.local','referral_sent') RETURNING id INTO _ref_email;
  INSERT INTO public.opportunity_applications(opportunity_id,recruiter_id,driver_user_id,application_type,status)
  ...
  INSERT INTO public._phase3c_results VALUES ('B email-only advances','application_started',_status,_status='application_started');
  INSERT INTO public._phase3c_results VALUES ('B email-only safe-links user','true',_has_user::text,_has_user);
  ...
  INSERT INTO public._phase3c_results VALUES ('C earliest credited','application_started',
  ...
  INSERT INTO public._phase3c_results VALUES ('C later NOT credited','referral_sent',
  ...
  INSERT INTO public._phase3c_results VALUES ('D bonus protected on INSERT bridge','eligible_for_bonus',
  ...
  INSERT INTO public._phase3c_results VALUES ('D bonus protected on rejection','eligible_for_bonus',
    (SELECT status FROM public.driver_referrals WHERE id=_ref_bonus),
    (SELECT status FROM public.driver_referrals WHERE id=_ref_bonus)='eligible_for_bonus');
  UPDATE public.opportunity_applications SET status='rejected' WHERE id=_app1;
  INSERT INTO public._phase3c_results VALUES ('D non-protected closes on reject','closed_not_hired',
