/**
 * Pure cost-profile math helpers. NO Supabase / React / React Query imports.
 *
 * Hooks (e.g. `useCostProfile`) import and re-export these so callers can
 * keep their existing import paths, while tests import directly from
 * `@/lib/costProfileMath` and run without instantiating the Supabase client.
 */
...
export type CostProfileUpdate = Partial<
  Omit<CostProfile, 'id' | 'user_id' | 'created_at' | 'updated_at'>
>;
 ...
 * profiles are intentionally included so the downstream
 * `fixed_missing_monthly_miles` warning can surface in the UI instead of
 * silently falling back to history.
 */
