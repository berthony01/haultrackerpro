...
// ---------------------------------------------------------------------------
// Phase 1J-C1 additive — REAL DriverOpportunityProfile mutation-failure
// contract: when useDriverOpportunityProfile().upsertProfile.mutate invokes
// onError (and never onSuccess), the parent-supplied onSaveSuccess MUST NOT
// be called. This exercises the real component (not a mocked boundary).
// ---------------------------------------------------------------------------

const realProfileMutate = vi.fn();
vi.mock('@/hooks/opportunities/useDriverOpportunityProfile', () => ({
  useDriverOpportunityProfile: () => ({
    ...
    error: null,
    refetch: vi.fn(),
    upsertProfile: { mutate: realProfileMutate, isPending: false },
    deleteProfile: { mutate: vi.fn(), isPending: false },
  }),
}));
vi.mock('@/hooks/useAuth', () => ({
  useAuth: () => ({
    user: { id: 'u-real', email: 'real@example.com', user_metadata: {} },
  }),
}));

describe('Phase 1J-C1 — REAL DriverOpportunityProfile mutation-failure contract', () => {
  it('when upsertProfile.mutate invokes onError, onSaveSuccess is never called', async () => {
    // Fresh dynamic import of the REAL component (this file does not mock it).
    const { DriverOpportunityProfile } = await import(
      '@/components/opportunities/DriverOpportunityProfile'
    );
    // Simulate a real Supabase failure: invoke onError, never onSuccess.
    realProfileMutate.mockImplementation((_payload: any, opts: any) => {
    ...
    render(
      <QueryClientProvider client={qc}>
        <DriverOpportunityProfile onBack={onBack} onSaveSuccess={onSaveSuccess} />
      </QueryClientProvider>,
    );
    // Fill only the fields that guarantee handleSave passes early validation
    // and reaches mutate. isComplete() may still return false, but that is
    // irrelevant — the mutation itself fails.
    await userEvent.type(screen.getByPlaceholderText('John Doe'), 'Jane Driver');
    await userEvent.type(screen.getByPlaceholderText('you@example.com'), 'jane@example.com');
    // Click Save Preferences (fixed action bar). There is only one such button.
    await userEvent.click(screen.getByRole('button', { name: /Save Preferences/i }));
    // mutate was invoked, onError fired, onSaveSuccess was NEVER called.
    expect(realProfileMutate).toHaveBeenCalledTimes(1);
    expect(onSaveSuccess).not.toHaveBeenCalled();
    // Back was not auto-triggered either — the parent's Apply resume flow
    // depends entirely on onSaveSuccess, so a failed mutation cannot revive it.
